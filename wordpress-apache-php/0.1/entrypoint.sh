...
load_phpmyadmin(){
        if ! grep -q "^Include httpd-phpmyadmin.conf" $HTTPD_CONF_FILE; then
                echo 'Include httpd-phpmyadmin.conf' >> $HTTPD_CONF_FILE
        fi
}
...
load_wordpress(){
    if ! grep -q "^Include httpd-wordpress.conf" $HTTPD_CONF_FILE; then
            echo 'Include httpd-wordpress.conf' >> $HTTPD_CONF_FILE
    fi
}
